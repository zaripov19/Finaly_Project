...
<div class="container mt-5">
    ...
    <%
    } else if ("noSeats".equals(errorMessage)) {
    %>
    <div class="alert alert-danger" role="alert">
        Sorry, no seats available for this event!
    </div>
    <%
    } else if ("eventNotFound".equals(errorMessage)) {
    %>
    <div class="alert alert-danger" role="alert">
        Event not found!
    </div>
    <%
    } else if ("serverError".equals(errorMessage)) {
    %>
    <div class="alert alert-danger" role="alert">
...
<script>
    // JavaScript to hide "Take a part" button after click and disable the form submission
    document.querySelectorAll('form').forEach(function (form) {
        form.addEventListener('submit', function (event) {
            var eventId = form.querySelector('input[name="eventId"]').value;
            var button = document.getElementById('btn_' + eventId);

            // Disable the button after click and hide it
